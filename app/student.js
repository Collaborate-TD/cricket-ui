    ...
    const fetchUser = async () => {
      try {
        ...
        const photoUrl = res.data?.profilePhoto
          ? `${process.env.API_URL || 'http://192.168.2.13:5000'}/data/profile/${res.data._id}/${res.data.profilePhoto}?t=${Date.now()}`
          : null;
        setProfilePhoto(photoUrl || defaultUser);
        setFirstName(res.data.firstName || 'Student');
      } catch (err) {
        //console.error('Failed to fetch user:', err.message);
        setProfilePhoto(defaultUser);
      } finally {
        setLoading(false);
      }
    };
        ...
        <View style={styles.profileContainer}>
          ...
          >
            <Image
              ...
              style={styles.avatar}
              resizeMode="cover"
              onError={() => setProfilePhoto(defaultUser)}
